...
import {
  ...
  makeDiagnostic,
  makeTiming,
} from "../src/services/factories";
import {
  parsePyDiagnostic,
  parseGccDiagnostics,
  parseTraceJson,
  buildTraceFile,
} from "../src/services/browserHelpers";
import { convertRustResult } from "../src/services/native";
import type { TraceFile, TraceStep } from "../src/types";
...
function expectMatch(name: string, actual: string, re: RegExp): void {
  if (re.test(actual)) {
    pass++;
    return;
  }
...
// makeTiming basic
{
  const t = makeTiming({ instrumentMs: 10, runMs: 20 });
...
{
  const t = makeTiming({ instrumentMs: 5, compileMs: 100, runMs: 15 });
...
// makeDiagnostic
{
  const d = makeDiagnostic("error", "test error", { line: 5, column: 3 });
...
// makeError
{
  const r = makeError({
    ...
    diagnostics: [makeDiagnostic("error", "SyntaxError", { line: 7 })],
    instrumentMs: 11,
  });
...
SyntaxError: invalid syntax`;
  const diags = parsePyDiagnostic(stderr);
...
source.cpp:3:1: warning: unused variable 'x' [-Wunused-variable]`;
  const diags = parseGccDiagnostics(stderr);
...
// buildTraceFile
{
  ...
  const tf = buildTraceFile("print(1)\nprint(2)", "python", steps);
