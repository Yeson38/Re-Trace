/**
 * Phase 3 pure analysis primitives.
 *
 * Every function here is TOTAL and PURE:
 *   - No DOM, no Player, no timers.
 *   - Inputs come from the adapter-emitted `TraceStep[]` / `TraceFile` protocol.
 *   - No exceptions for malformed search predicates or missing vars;
 *     we return error structs or empty results.
 *
 * These properties let us run the test suite in Node without a browser
 * and also guarantee that analysis never breaks playback.
 */
import type { TraceFile, TraceStep, TraceVar } from "./types";

// ---------------------------------------------------------------------------
// 1. Timeline search (binary-friendly)
// ---------------------------------------------------------------------------

export type SearchOp = "==" | "!=" | ">" | "<" | ">=" | "<=" | "~=";
// ~= means "loose contains / fuzzy match" (substring for strings, in for array/list,
// key-exists for objects).

export interface SearchPredicate {
  /** Variable path. Accepts dotted `foo.bar` or indexed `data[3]` forms. */
  name: string;
  op: SearchOp;
  /** Raw target value as parsed. Always JSON; we try JSON.parse first, else string literal. */
  value: unknown;
  /** The original user-written query string (for UI feedback). */
  raw: string;
}

export interface ParseError {
  error: string;
  /** 0-based column hint for where parsing failed, or -1 if unknown. */
  column: number;
}
 ...
 *   name == "hello"
 */
export function parseSearch(input: string): SearchPredicate | ParseError {
  const raw = input.trim();
  if (!raw) return { error: "empty query", column: -1 };

  const m = raw.match(OP_REGEX);
  if (!m || m.index === undefined) {
    return { error: "missing operator, expected one of == != > < >= <= ~=", column: raw.length };
  }
  const op = m[1] as SearchOp;
  const namePart = raw.slice(0, m.index).trim();
  const valPart = raw.slice(m.index + op.length).trim();

  const nameErr = validatePath(namePart);
  if (nameErr) return { error: nameErr, column: 0 };

  const valRes = parseLiteral(valPart);
  if ("error" in valRes) return { error: valRes.error, column: m.index + op.length + 1 + (valRes.column ?? 0) };

  return { name: namePart, op, value: valRes.value, raw };
}

function validatePath(p: string): string | null {
  if (!p) return "empty variable path";
  // Identifier-like start, then dotted identifiers or [literal].
  const re = /^[A-Za-z_][A-Za-z0-9_]*(?:\.[A-Za-z_][A-Za-z0-9_]*|\[(?:-?\d+|"[^"]*"|'[^']*')\])*$/;
  if (!re.test(p)) return `invalid variable path: ${JSON.stringify(p)}`;
  return null;
}

function parseLiteral(raw: string): { value: unknown } | { error: string; column?: number } {
  if (raw === "") return { error: "missing value on the right-hand side" };
  if (raw === "true") return { value: true };
  if (raw === "false") return { value: false };
  if (raw === "null" || raw === "None") return { value: null };
  // Quoted string.
  if ((raw.startsWith('"') && raw.endsWith('"')) || (raw.startsWith("'") && raw.endsWith("'"))) {
    if (raw.length === 1) return { error: "unterminated string literal" };
...
export function resolvePath(vars: TraceVar[], path: string): { found: true; value: unknown } | { found: false } {
  ...
  if (first.kind !== "ident") return { found: false };
  let cur: unknown = undefined;
  let have = false;
  for (const v of vars) {
    if (v.name === first.name) { cur = v.value; have = true; break; }
  }
  if (!have) return { found: false };
  ...
  for (let i = 1; i < tokens.length; i += 1) {
    ...
    if (t.kind === "ident") {
      ...
      if (!(t.name in rec)) return { found: false };
      cur = rec[t.name];
    } else if (t.kind === "index-num") {
      if (Array.isArray(cur)) {
        const arr = cur as unknown[];
        const idx = t.index < 0 ? arr.length + t.index : t.index;
        if (idx < 0 || idx >= arr.length) return { found: false };
        cur = arr[idx];
      } else if (typeof cur === "object") {
        const key = String(t.index);
        const rec = cur as Record<string, unknown>;
        if (!(key in rec)) return { found: false };
        cur = rec[key];
      } else {
        return { found: false };
      }
      ...
      if (typeof cur !== "object") return { found: false };
      const rec = cur as Record<string, unknown>;
      if (!(t.index in rec)) return { found: false };
      cur = rec[t.index];
    }
  }
...
type PathTok =
  ...
  | { kind: "index-num"; index: number }
  | { kind: "index-str"; index: string };

function tokenizePath(p: string): PathTok[] {
  const out: PathTok[] = [];
  let i = 0;
  while (i < p.length) {
    const c = p[i];
    if (c === ".") {
      ...
      // Identifier expected.
      let n = 0;
      while (i + n < p.length && /[A-Za-z0-9_]/.test(p[i + n])) n += 1;
...
function numCoerce(a: unknown, b: unknown): [number, number] | null {
...
export function matches(pred: SearchPredicate, step: TraceStep): boolean {
  ...
  const eq = (x: unknown, y: unknown): boolean => {
    ...
    if (typeof x === "object" && x !== null && typeof y === "object" && y !== null) {
      const xk = Object.keys(x as Record<string, unknown>).sort();
      const yk = Object.keys(y as Record<string, unknown>).sort();
      if (xk.length !== yk.length) return false;
      if (!xk.every((k, i) => k === yk[i])) return false;
      return xk.every((k) => eq((x as Record<string, unknown>)[k], (y as Record<string, unknown>)[k]));
    }
    return false;
  };
  ...
  switch (pred.op) {
    ...
    case "~=": {
      ...
      if (Array.isArray(a)) return a.some((v) => eq(v, b));
      if (typeof a === "string") return a.includes(String(b ?? ""));
      if (a !== null && typeof a === "object") return String(b) in (a as Record<string, unknown>);
      return false;
    }
  }
}
 ...
 */
export function findFirstMatch(steps: TraceStep[], pred: SearchPredicate, from = 0): number {
  const start = Math.max(0, Math.min(steps.length - 1, from));
  for (let i = start; i < steps.length; i += 1) {
    if (matches(pred, steps[i])) return i;
  }
  return -1;
}

/** Find every step index that satisfies the predicate. */
export function findAllMatches(steps: TraceStep[], pred: SearchPredicate): number[] {
  const out: number[] = [];
  for (let i = 0; i < steps.length; i += 1) {
    if (matches(pred, steps[i])) out.push(i);
  }
  return out;
}
...
export interface ComplexityEstimate {
  order: string;   // e.g. "O(n)", "O(n²)", "O(log n)", "O(1)", "O(2ⁿ)", "O(?)"
 ...
 */
export function estimateLoopComplexity(file: TraceFile, loopId: string): ComplexityEstimate {
  const steps = file.steps;
  const active: TraceStep[] = steps.filter((s) => s.loops.some((l) => l.id === loopId));
  if (active.length === 0) {
  ...
  for (const s of active) {
    if (s.loops.length > maxDepth) maxDepth = s.loops.length;
  }
  ...
  for (const s of active) {
    ...
    if (depthIn > innermostPos) innermostPos = depthIn;
  }
  ...
  if (last.vars.some((v) => /^(lo|low|left|hi|high|right|mid|middle)$/.test(v.name.toLowerCase()))) {
    ...
    if (rangeStart > 8 && rangeEnd > 0 && rangeEnd < rangeStart / 2) logHeuristic = true;
  }
...
function numericRangeSize(vars: TraceVar[]): number {
...
export function estimateProgram(file: TraceFile): ComplexityEstimate {
  const loopIds = new Set<string>();
  for (const s of file.steps) for (const l of s.loops) loopIds.add(l.id);
  if (loopIds.size === 0) {
    return { order: "O(1)", basis: "no loops visible", confidence: "high" };
  }
  const worst: { est: ComplexityEstimate; id: string; rank: number }[] = [];
...
function rankOrder(o: string): number {
  // Higher = asymptotically larger; unknowns rank lowest so we don't overclaim.
  if (o.includes("2ⁿ") || o.includes("2^n")) return 100;
  if (o.includes("n!") || o.includes("fact")) return 90;
  if (o.includes("n³") || o.includes("n^3")) return 80;
  if (o.includes("n²") || o.includes("n^2")) return 70;
  if (o.includes("n log")) return 60;
  if (o.includes("n")) return 50;
  if (o.includes("log")) return 40;
  if (o.includes("1)")) return 30;
  return 0; // O(?)
}

// ---------------------------------------------------------------------------
// 3. Variable diff between two step-snapshots
// ---------------------------------------------------------------------------
...
export interface VarDiff {
  name: string;
  kind: DiffKind;
  /** Top-level only has a single type; compound children also typed. */
  type?: string;
  /** For added, removed, modified primitives. */
  before?: unknown;
  after?: unknown;
  /** For compound values, recursive differences. */
  children?: VarDiff[];
}
 ...
 */
export function diffVars(before: TraceVar[], after: TraceVar[]): VarDiff[] {
...
function valueDiff(name: string, b: unknown, a: unknown, bType?: string, aType?: string): VarDiff {
  ...
  if (Array.isArray(a) || Array.isArray(b)) {
    ...
    for (let i = 0; i < len; i += 1) {
      if (i >= bArr.length) kids.push({ name: String(i), kind: "added", after: aArr[i] });
      else if (i >= aArr.length) kids.push({ name: String(i), kind: "removed", before: bArr[i] });
      else kids.push(valueDiff(String(i), bArr[i], aArr[i]));
    }
    // If all children unchanged → overall unchanged.
    if (same) res.kind = "unchanged";
    res.children = kids;
    return res;
  }
  if (b !== null && a !== null && typeof b === "object" && typeof a === "object" && !Array.isArray(a) && !Array.isArray(b)) {
    ...
    const kids: VarDiff[] = [];
    for (const k of keys) {
      if (!(k in bRec)) kids.push({ name: k, kind: "added", after: aRec[k] });
      else if (!(k in aRec)) kids.push({ name: k, kind: "removed", before: bRec[k] });
      else kids.push(valueDiff(k, bRec[k], aRec[k]));
    }
    res.children = kids;
    if (same) res.kind = "unchanged";
    return res;
...
function equalForDiff(x: unknown, y: unknown): boolean {
  if (Object.is(x, y)) return true;
  if (Array.isArray(x) && Array.isArray(y)) {
    if (x.length !== y.length) return false;
  ...
  if (x !== null && y !== null && typeof x === "object" && typeof y === "object" && !Array.isArray(x) && !Array.isArray(y)) {
    ...
    xk.sort(); yk.sort();
    if (!xk.every((k, i) => k === yk[i])) return false;
    return xk.every((k) => equalForDiff((x as Record<string, unknown>)[k], (y as Record<string, unknown>)[k]));
  }
  return false;
}
